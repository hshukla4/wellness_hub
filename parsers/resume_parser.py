...
def parse_resume(filename):
    ...
    try:
        ...
        returned_value = e
    print(json.dumps(returned_value, indent=4))
...
def _extract_education(cleaned_text):
    extract_education = [
        kw for kw in education_keywords if kw.lower() in cleaned_text.lower()
    ]
...
def _extract_experience(cleaned_text):
    match = re.search(
        r"(Experience:|Work Experience:|Job Title:)(.*)", cleaned_text, re.IGNORECASE
    )
    if match:
